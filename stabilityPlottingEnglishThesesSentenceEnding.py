...
def plot_scatterplot():
    for thesis in all_theses:
        ...
        plt.scatter(this_doc['length_of_chunk'], this_doc['GFI'], color = 'orange', label = 'with ";" and ":" not end of sentence')
        plt.scatter(this_doc_shortened['length_of_chunk'], this_doc_shortened['GFI'], color = 'green', label = 'with ";" and ":" end of sentence')
...
def plot_deviations():
    ...
    for i in range(len(chunk_sizes_shortened)):
        colors.append("green")
    plt.scatter(x_s, y_s, color = colors)
...
df_shortened = pd.read_csv("Results/English/resultsThesesShortenedSentences.csv", header=0, index_col=0)
...
# the theses where ";" and ":" have been replaced by "."
df_variances_shortened = pd.read_csv("Results/English/variancesThesesShortenedSentences.csv", header=0, index_col=0)
